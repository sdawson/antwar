...
def updateGrid(grid, birthProb, redMajorProb,
    blueMajorProb, stats, check="nodiag"):
  newGrid = grid.copy()
  (r, c) = grid.shape
  for i in range(r):
    for j in range(c):
      if grid[i][j] == EMPTY:
        newGrid[i][j] = maybePopulateCell(birthProb,
            redMajorProb, blueMajorProb)
      else:
        newGrid[i][j] = updateCell(grid, i, j, stats,
            birthProb, redMajorProb, blueMajorProb, check)
  return newGrid
