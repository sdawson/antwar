...
stats = mp.Array('i', [0,0,0,0,0], lock=True)
...
def main():
  ...
  #    "majblue": 0,
  #    "majred": 0}
  # minblue, minred, majblue, majred
  # set up to match the indices used as cell constants
  #empty_stats = [0,0,0,0,0]
  #stats = mp.Array('i', empty_stats, lock=True)
  ...
  print "%d\t%d\t%d\t%d\t%f\t%f" % (stats[REDMAJOR] + stats[BLUEMAJOR],
                                       stats[REDMINOR] + stats[BLUEMINOR],
                                       stats[REDMINOR] + stats[REDMAJOR],
                                       stats[BLUEMINOR] + stats[BLUEMAJOR],
                                       (stats[REDMAJOR] + stats[BLUEMAJOR])/noOfSteps,
                                       (stats[REDMINOR] + stats[BLUEMINOR])/noOfSteps)
  colorama.deinit()
...
def updateGrid(grid, birthProb, majorProb, check="nodiag"):
  #newGrid = grid.copy()
  (r, c) = grid.shape
  inds = numpy.rollaxis(numpy.indices((r, c), dtype=numpy.int), 0, 3).reshape(-1, 2)

  partialUpdateCell = partial(updateCell, grid, birthProb, majorProb, check)
  pool = mp.Pool()
  results = pool.map(partialUpdateCell, inds)
...
def updateCell(grid, birthProb, majorProb, check, r_c):
  [r, c] = r_c
  if grid[r][c] == REDMINOR:
    isDeath = isDeathCondition(grid, r, c, check, 1, 1)
    if isDeath:
      # minred + 1
      stats[REDMINOR] = stats[REDMINOR] + 1 # +1 to no. of minor ant deaths
    ...
    if isDeath:
      # minblue
      stats[BLUEMINOR] = stats[BLUEMINOR] + 1
      return [fillAntDeathCell(grid, r, c, isDeath, birthProb, majorProb), r, c]
    else:
      return [grid[r][c], r, c]
    ...
    if isDeath:
      # majred
      stats[REDMAJOR] = stats[REDMAJOR] + 1 # +1 to no. of major ant deaths
      return [fillAntDeathCell(grid, r, c, isDeath, birthProb, majorProb), r, c]
    else:
      return [grid[r][c], r, c]
    ...
    if isDeath:
      # majblue
      stats[BLUEMAJOR] = stats[BLUEMAJOR] + 1 # +1 to no. of major ant deaths
      return [fillAntDeathCell(grid, r, c, isDeath, birthProb, majorProb), r, c]
    else:
      return [grid[r][c], r, c]
  elif grid[r][c] == EMPTY:
